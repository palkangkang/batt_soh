# 因果效应报告：初始充电倍率对未来相对容量下降的影响

## 1. 分析设定
- 运行时间：2026-03-28 16:13:20
- Python解释器：`C:\Users\pal\.virtualenvs\colab-OixbOpvz\Scripts\python.EXE`
- 预测窗口长度（horizon_cycles）：`200`
- 处理变量定义（treatment_mode）：初始充电倍率（initial）
- 排除策略前缀（exclude_policy_prefix）：`VARCHARGE`

## 2. 主要结论
- 全局 +1C 效应：**0.013829** （95%置信区间：0.010753 ~ 0.017071)

## 3. 分段方案并行结果
### 3.1 固定分段4段（0-500 / 500-1000 / 1000-1500 / 1500-max）
| 分组 | effect_plus_1c | ci_low | ci_high | n_rows | n_clusters | bootstrap_success |
|---|---:|---:|---:|---:|---:|---:|
| 0-500 | 0.009639 | 0.007149 | 0.012588 | 72096 | 180 | 400 |
| 500-1000 | 0.025763 | 0.020672 | 0.030379 | 24133 | 92 | 400 |
| 1000-1500 | 0.033651 | 0.005927 | 0.052755 | 3924 | 14 | 400 |
| 1500-max | 0.025367 | -0.053641 | 0.050520 | 1496 | 5 | 400 |

### 3.2 分位数分段4段（Q1 / Q2 / Q3 / Q4）
| 分组 | effect_plus_1c | ci_low | ci_high | n_rows | n_clusters | bootstrap_success |
|---|---:|---:|---:|---:|---:|---:|
| Q1 | 0.006820 | 0.004831 | 0.008601 | 25503 | 180 | 400 |
| Q2 | 0.003952 | -0.001184 | 0.009631 | 25301 | 178 | 400 |
| Q3 | 0.022305 | 0.018109 | 0.027139 | 25382 | 141 | 400 |
| Q4 | 0.025775 | 0.019240 | 0.031298 | 25463 | 83 | 400 |
...
| 分位数分段4段 | Q1 | 25503 | 180 | 1 | 144 |
| 分位数分段4段 | Q2 | 25301 | 178 | 144 | 295 |
| 分位数分段4段 | Q3 | 25382 | 141 | 295 | 547 |
| 分位数分段4段 | Q4 | 25463 | 83 | 547 | 2037 |

### 3.5 两方案量化对比结论
- 阶段效应方向一致性：固定分段正向阶段 `4/4`，分位数分段正向阶段 `4/4`，跨方案一致性约 `100.00%`（全部正向）。
- 首末阶段效应差：固定分段 `1500-max - 0-500 = 0.015728`；分位数分段 `Q4 - Q1 = 0.018955`。
- 不确定性对比（CI宽度）：固定分段均值 `0.041534`、最大 `0.104162`；分位数分段均值 `0.008918`、最大 `0.012058`。
- 样本均衡性（n_rows离散度）：固定分段 CV=`1.1156`、max/min=`48.19`；分位数分段 CV=`0.0031`、max/min=`1.01`。
...
### 5.1 固定分段4段（4张）
#### 图1：剂量-反应曲线（固定分段4段）
![图1 剂量-反应曲线](./fig_dose_response_initial.png)
- X轴含义：处理强度 `t_eval`（充电倍率 C）。
- Y轴含义：`E[Y | do(T=t)]`，即在干预到倍率 t 时的未来窗口相对容量下降期望。
- 关键性结论：固定分段全局曲线范围约 `0.03032 ~ 0.10073`，说明不同倍率下损失水平存在系统差异。
- 业务解释：这张图回答“把倍率设为某个具体值时，预期衰减水平是多少”。

#### 图2：全局与分阶段 +1C 效应（固定分段4段）
![图2 全局与分阶段+1C效应](./fig_plus1c_by_stage_initial.png)
- X轴含义：分组（全局、0-500、500-1000、1000-1500、1500-max）。
- Y轴含义：`+1C` 对未来相对容量下降的增量影响。
- 关键性结论：固定分段后段效应（1500-max）约 `0.025367`。
- 业务解释：同样增加 1C，在寿命后段带来的额外损失更明显。

#### 图3：+1C 局部效应曲线（固定分段4段）
![图3 +1C局部效应曲线](./fig_delta_plus1c_curve_initial.png)
- X轴含义：基准处理强度 `base_t`（当前倍率）。
- Y轴含义：`m(t+1)-m(t)`，即在该基准倍率处再提高 1C 的局部影响。
- 关键性结论：固定分段下，全局局部效应均值约 `0.012571`，区间约 `-0.004738 ~ 0.033797`。
- 业务解释：这张图回答“在不同当前倍率下，再加 1C 的边际代价是否一致”。

#### 图4：结果变量分阶段分布（固定分段4段）
![图4 结果变量分阶段分布（固定分段）](./fig_outcome_by_stage_initial.png)
- X轴含义：寿命阶段（固定分段4段）。
...
### 5.2 分位数分段4段（4张）
#### 图5：剂量-反应曲线（分位数分段4段）
![图5 剂量-反应曲线（分位数）](./fig_dose_response_quantile4_initial.png)
- X轴含义：处理强度 `t_eval`（充电倍率 C）。
- Y轴含义：`E[Y | do(T=t)]`，即在干预到倍率 t 时的未来窗口相对容量下降期望。
- 关键性结论：分位数分段全局曲线范围约 `0.03032 ~ 0.10073`。
- 业务解释：用于在样本更均衡分段下复核倍率干预的整体趋势。

#### 图6：全局与分阶段 +1C 效应（分位数分段4段）
![图6 全局与分阶段+1C效应（分位数）](./fig_plus1c_by_stage_quantile4_initial.png)
- X轴含义：分组（全局、Q1、Q2、Q3、Q4）。
- Y轴含义：`+1C` 对未来相对容量下降的增量影响。
- 关键性结论：分位数分段后段效应（Q4）约 `0.025775`。
- 业务解释：在样本量更均衡的阶段定义下，后段仍表现出更高边际损失。

#### 图7：+1C 局部效应曲线（分位数分段4段）
![图7 +1C局部效应曲线（分位数）](./fig_delta_plus1c_curve_quantile4_initial.png)
- X轴含义：基准处理强度 `base_t`（当前倍率）。
- Y轴含义：`m(t+1)-m(t)`，即在该基准倍率处再提高 1C 的局部影响。
- 关键性结论：分位数分段下，全局局部效应均值约 `0.012571`，区间约 `-0.004738 ~ 0.033797`。
- 业务解释：用于检验在分位数分段口径下，边际代价曲线是否与固定分段一致。

#### 图8：结果变量分阶段分布（分位数分段4段）
![图8 结果变量分阶段分布（分位数）](./fig_outcome_by_stage_quantile4_initial.png)
- X轴含义：寿命阶段（分位数分段4段，Q1~Q4）。
- Y轴含义：`y_rel_drop`，即未来窗口相对容量下降。
- 关键性结论：分位数分段下，Q4 阶段均值相对 Q1 阶段变化 `0.027058`。
- 业务解释：在等样本量切分下比较衰减分布，减少样本不均衡干扰。

### 5.3 通用诊断图（2张）
#### 图9：处理变量分布与支持区间
![图9 处理变量分布](./fig_treatment_distribution_initial.png)
...
#### 图10：权重与有效样本量诊断
![图10 权重与有效样本量诊断](./fig_weight_diagnostics_initial.png)
- X轴含义：评估处理强度 `t_eval`（充电倍率 C）。
- Y轴含义：左轴为 ESS（有效样本量），右轴为高分位权重诊断（`weight_p99`、`weight_max`）。
- 关键性结论：最小 ESS 约 `2353.54`（出现在 t≈8.00），最大 p99 权重约 `153.06`。
- 业务解释：ESS 过低或高分位权重过大时，局部估计的不确定性会增加。

### 5.4 方案对比图（1张）
#### 图11：固定分段与分位数分段阶段效应对比
![图11 两种分段方案阶段效应对比](./fig_plus1c_by_stage_compare_initial.png)
- X轴含义：`scheme × stage` 组合分组（固定4段与分位数4段）。
- Y轴含义：各阶段 `+1C` 对未来相对容量下降的估计影响（含置信区间）。
- 关键性结论：固定分段首末差 `0.015728`、分位数分段首末差 `0.018955`，且分位数分段 CI 宽度均值更小（`0.008918` vs `0.041534`）。
