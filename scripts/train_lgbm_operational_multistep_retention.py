...
DEFAULT_OUTPUT_DIR = REPO_ROOT / "outputs" / "analysis" / "lgbm_operational_multistep_retention"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="LightGBM multistep retention using 5 operational feature groups (no policy params)."
    )
    ...
    parser.add_argument("--window-mode", choices=["rolling", "fixed_origin", "fixed_blocks"], default="rolling")
...
def aggregate_history_features(hist_view: np.ndarray, n_history: int) -> np.ndarray:
    """Aggregate history windows into last/mean/std/slope feature blocks."""
    if hist_view.ndim != 3:
...
def compute_group_window_pack(
    ...
    x_aggr_all = aggregate_history_features(hist_view=hist_view, n_history=n_history)
...
def compute_group_fixed_origin_pack(
    part: pd.DataFrame,
    raw_feature_cols: Sequence[str],
    n_history: int,
    horizon_steps: int,
) -> Optional[WindowPack]:
    """Build one fixed-origin sample using cycles 1:N to predict N+1:N+M."""
    expected_cycles = np.arange(1, n_history + horizon_steps + 1, dtype=np.int64)
    fixed = part.loc[part["cycles"].isin(expected_cycles)].sort_values("cycles", kind="mergesort").copy()
    cycles = fixed["cycles"].to_numpy(dtype=np.int64)
    if len(cycles) != len(expected_cycles) or not np.array_equal(cycles, expected_cycles):
        return None

    hist_raw = fixed.iloc[:n_history][list(raw_feature_cols)].to_numpy(dtype=np.float32)
    target = fixed.iloc[n_history : n_history + horizon_steps]["retention"].to_numpy(dtype=np.float32)
    target_cycles = fixed.iloc[n_history : n_history + horizon_steps]["cycles"].to_numpy(dtype=np.int64)
    x_aggr = aggregate_history_features(hist_view=hist_raw[np.newaxis, :, :], n_history=n_history)

    policy = str(part.iloc[0]["policy"])
    cell_code = str(part.iloc[0]["cell_code"])
    group_key = f"{policy}||{cell_code}"
    return WindowPack(
        x=x_aggr.astype(np.float32),
        y=target[np.newaxis, :].astype(np.float32),
        group_keys=np.array([group_key], dtype=object),
        input_cycles=np.array([n_history], dtype=np.int64),
        target_cycles=target_cycles[np.newaxis, :].astype(np.int64),
    )


def compute_group_fixed_blocks_pack(
    part: pd.DataFrame,
    raw_feature_cols: Sequence[str],
    n_history: int,
    horizon_steps: int,
) -> Optional[WindowPack]:
    """Build fixed-block samples using starts 1, 151, and 301."""
    x_rows: List[np.ndarray] = []
    y_rows: List[np.ndarray] = []
    input_cycles: List[int] = []
    target_cycles: List[np.ndarray] = []

    cycle_index = part.sort_values("cycles", kind="mergesort").set_index("cycles", drop=False)
    for start_cycle in FIXED_BLOCK_STARTS:
        input_start = int(start_cycle)
        input_end = input_start + n_history - 1
        target_start = input_end + 1
        target_end = target_start + horizon_steps - 1
        expected_cycles = np.arange(input_start, target_end + 1, dtype=np.int64)
        if not set(expected_cycles).issubset(set(cycle_index.index.astype(int).tolist())):
            continue

        block = cycle_index.loc[expected_cycles].reset_index(drop=True)
        cycles = block["cycles"].to_numpy(dtype=np.int64)
        if len(cycles) != len(expected_cycles) or not np.array_equal(cycles, expected_cycles):
            continue

        hist_raw = block.iloc[:n_history][list(raw_feature_cols)].to_numpy(dtype=np.float32)
        target = block.iloc[n_history : n_history + horizon_steps]["retention"].to_numpy(dtype=np.float32)
        target_cycle_row = block.iloc[n_history : n_history + horizon_steps]["cycles"].to_numpy(dtype=np.int64)
        x_rows.append(aggregate_history_features(hist_view=hist_raw[np.newaxis, :, :], n_history=n_history)[0])
        y_rows.append(target.astype(np.float32))
        input_cycles.append(input_end)
        target_cycles.append(target_cycle_row.astype(np.int64))
    ...
    return WindowPack(
        ...
        group_keys=np.full(len(x_rows), group_key, dtype=object),
        input_cycles=np.asarray(input_cycles, dtype=np.int64),
        target_cycles=np.vstack(target_cycles).astype(np.int64),
    )
...
def build_train_valid_windows(
    ...
    for (policy, cell_code), part in merged.groupby(["policy", "cell_code"], sort=False):
        ...
        elif window_mode == "fixed_origin":
            wp = compute_group_fixed_origin_pack(
                part=part,
                raw_feature_cols=raw_feature_cols,
                n_history=n_history,
                horizon_steps=horizon_steps,
            )
        elif window_mode == "fixed_blocks":
            wp = compute_group_fixed_blocks_pack(
                part=part,
                raw_feature_cols=raw_feature_cols,
                n_history=n_history,
                horizon_steps=horizon_steps,
    ...
    counters["window_mode"] = str(window_mode)
...
def build_dataset_checks(
    ...
    expected_fixed_target = np.arange(n_history + 1, n_history + horizon_steps + 1, dtype=np.int64)
    check_fixed_input = bool(np.all(all_input == n_history)) if len(all_input) > 0 else False
    check_fixed_target = bool(np.all(all_target == expected_fixed_target.reshape(1, -1))) if len(all_target) > 0 else False
    block_starts = np.asarray(FIXED_BLOCK_STARTS, dtype=np.int64)
    allowed_block_inputs = block_starts + n_history - 1
    allowed_block_target_starts = block_starts + n_history
    check_blocks_input_length = bool(all_target.shape[1] == horizon_steps) if len(all_target) > 0 else False
    check_blocks_target_length = bool(all_target.shape[1] == horizon_steps) if len(all_target) > 0 else False
    check_blocks_after_input = bool(np.all(all_input < all_target[:, 0])) if len(all_input) > 0 else False
    check_blocks_allowed_input = bool(np.all(np.isin(all_input, allowed_block_inputs))) if len(all_input) > 0 else False
    check_blocks_allowed_target = bool(np.all(np.isin(all_target[:, 0], allowed_block_target_starts))) if len(all_target) > 0 else False
    ...
    if window_mode == "fixed_origin":
        rows.extend(
            [
                {
                    "check_item": "check_fixed_origin_input_1_to_N",
                    "pass_flag": int(check_fixed_input),
                    "value": int(check_fixed_input),
                },
                {
                    "check_item": "check_fixed_origin_target_N_plus_1_to_N_plus_M",
                    "pass_flag": int(check_fixed_target),
                    "value": int(check_fixed_target),
                },
            ]
        )
    if window_mode == "fixed_blocks":
        rows.extend(
            [
                {
                    "check_item": "check_fixed_blocks_input_length_N",
                    "pass_flag": int(check_blocks_input_length),
                    "value": int(n_history),
                },
                {
                    "check_item": "check_fixed_blocks_target_length_M",
                    "pass_flag": int(check_blocks_target_length),
                    "value": int(horizon_steps),
                },
                {
                    "check_item": "check_fixed_blocks_target_after_input",
                    "pass_flag": int(check_blocks_after_input),
                    "value": int(check_blocks_after_input),
                },
                {
                    "check_item": "check_fixed_blocks_allowed_input_cycles",
                    "pass_flag": int(check_blocks_allowed_input),
                    "value": int(check_blocks_allowed_input),
                },
                {
                    "check_item": "check_fixed_blocks_allowed_target_starts",
                    "pass_flag": int(check_blocks_allowed_target),
                    "value": int(check_blocks_allowed_target),
                },
            ]
        )
...
def write_predictions_long_csv(
    ...
    def write_one(set_type: str, pack: WindowPack, pred: np.ndarray, mode: str, header: bool) -> None:
        ...
        group_key = np.repeat(pack.group_keys.astype(object), m)
...
def fit_lgbm_multioutput(
    ...
    """Train one LightGBM regressor per forecast horizon with progress logs."""
    estimators: List[LGBMRegressor] = []
    total = int(y_train.shape[1])
    for idx in range(total):
        start = time.perf_counter()
        estimator = LGBMRegressor(**dict(model_params))
        estimator.fit(x_train, y_train[:, idx])
        elapsed = time.perf_counter() - start
        estimators.append(estimator)
        print(f"Trained LightGBM horizon {idx + 1}/{total} in {elapsed:.1f}s", flush=True)
    return estimators


def predict_lgbm_multioutput(estimators: Sequence[LGBMRegressor], x_data: np.ndarray) -> np.ndarray:
    """Predict all horizons and return a two-dimensional float32 matrix."""
    preds = [estimator.predict(x_data, validate_features=False).astype(np.float32) for estimator in estimators]
...
def build_report(
    ...
    mode_name = mode_names.get(str(args.window_mode), str(args.window_mode))
    lines.append(f"# LightGBM 多步容量保持率预测报告（{mode_name}，5类工况特征）")
    lines.append("")
    lines.append("## 1. 运行摘要")
    ...
    lines.append(f"- 窗口模式：`{args.window_mode}`")
    ...
    lines.append("")
    if str(args.window_mode) == "fixed_origin":
        lines.append("固定起点说明：每个 `policy+cell_code` 最多贡献 1 个样本，输入严格为 cycles `1:N`，目标严格为 cycles `N+1:N+M`。")
        lines.append("")
    if str(args.window_mode) == "fixed_blocks":
        lines.append("分段固定起点说明：每个 `policy+cell_code` 最多贡献 3 个样本，默认 block 起点为 `1,151,301`。")
        lines.append("本次 `N=100,M=50` 时，对应 `1:100 -> 101:150`、`151:250 -> 251:300`、`301:400 -> 401:450`。")
        lines.append("")
    ...
    lines.append("## 5. 模型配置")
    lines.append("- 模型：每个 horizon 独立训练一个 `LGBMRegressor`")
    lines.append(f"- n_estimators：`{int(args.n_estimators)}`")
    lines.append(f"- learning_rate：`{float(args.learning_rate)}`")
    lines.append(f"- num_leaves/max_depth：`{int(args.num_leaves)}` / `{int(args.max_depth)}`")
    lines.append(f"- min_child_samples：`{int(args.min_child_samples)}`")
    lines.append(f"- subsample/colsample_bytree：`{float(args.subsample)}` / `{float(args.colsample_bytree)}`")
    lines.append(f"- reg_alpha/reg_lambda：`{float(args.reg_alpha)}` / `{float(args.reg_lambda)}`")
    lines.append(f"- n_jobs：`{int(args.n_jobs)}`")
    lines.append("")
    ...
    if str(args.window_mode) == "fixed_origin":
        lines.append("- 固定起点下每个电芯只有一个窗口，weighted 与 group-macro 更接近等权电芯评估。")
    elif str(args.window_mode) == "fixed_blocks":
        lines.append("- 分段固定起点扩大了每个电芯的可用样本，同时仍避免滑窗的大量重叠样本。")
    else:
        lines.append("- 若 weighted 与 group-macro 差距较大，优先以 group-macro 结论为准。")
    lines.append("")

    lines.append("## 7. 数据一致性检查")
...
def main() -> None:
    ...
    x_train = imputer.fit_transform(train_pack.x.astype(np.float32)).astype(np.float32)
    x_valid = imputer.transform(valid_pack.x.astype(np.float32)).astype(np.float32)
    estimators = fit_lgbm_multioutput(
        x_train=x_train,
        y_train=train_pack.y.astype(np.float32),
        model_params=model_params,
    )
    train_pred = predict_lgbm_multioutput(estimators, x_train)
    valid_pred = predict_lgbm_multioutput(estimators, x_valid)
    ...
    fi = np.mean([est.feature_importances_.astype(float) for est in estimators], axis=0)
    ...
    out_report = args.output_dir / "lgbm_operational_multistep_retention_report.md"
    ...
    append_session_log(
        prompt_text="实现LightGBM多步预测任务（5类工况特征，N/M可配置，无泄露）并执行冒烟+全量运行。",
        response_text=(
            f"新增脚本 {SCRIPT_PATH.name}，完成5类特征口径、滑窗多步LightGBM训练与输出；"
